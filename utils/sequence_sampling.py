...
def top_p_decode(model, device, max_len, start_symbol_ind=0, batch_size=1, p=0.95):
    # https://github.com/OpenNMT/OpenNMT-py/blob/83c85b80a257e0756b4ee593d27450297b92f88f/onmt/translate/greedy_search.py
    ys = torch.ones(batch_size, 1).fill_(start_symbol_ind).long().to(device)  # ys_0: (batch_size,T_pred=1)

    for i in range(max_len):
        # ys_i:(batch_size, T_pred=i+1)
        out = model(ys)  # (T_out, batch_size, nhid)
        prob = F.softmax(out[-1, :], dim=-1)  # (T_-1, batch_size, nhid)
        sorted_logits, sorted_indices = torch.sort(prob,
                                                   descending=True,
                                                   dim=1)
        cumulative_probs = torch.cumsum(sorted_logits, dim=-1)
        sorted_indices_to_keep = cumulative_probs.lt(p)
        ...
        prob = prob.masked_fill(~keep_indices, 0)

        next_word = torch.multinomial(prob, 1)  # (batch_size, 1)
